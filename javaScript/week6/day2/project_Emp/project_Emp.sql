...
-- (
--    Deptcode   INTEGER NOT NULL PRIMARY KEY default nextval('serial_seq'),
--    DeptName   CHAR(30),
...
SELECT COUNT(*) FROM employee WHERE job = 'ANALYST'

-- 3. Display the names of each employees, their job and dept location
SELECT empfname,job,location FROM employee
...
SELECT empfname,empfname,departmentcode,deptname,location FROM employee
INNER JOIN department ON employee.departmentcode=department.deptcode
ORDER BY empfname,location ASC

-- 7. Display EMPFNAME and "TOTAL SALARY" for each employee (commission and salary)
SELECT empfname,(salary + commission) AS sum FROM employee 
