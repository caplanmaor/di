...
def posts():
    if current_user.is_authenticated:
        ...
        for post in posts:
            ...
            post_body = post.body
            author = db.session.query(Users).join(Profiles, Users.id == Profiles.user_id).join(Posts, Profiles.id==Posts.profile_id).filter_by(id=post.profile_id).first()
            post_object = {'title': post_title, 'body': post_body, 'user': author.name}
